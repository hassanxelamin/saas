...
import { SignUpCode } from '@/src/components/auth/sign-up/sign-up-code';
...
const SignUpForm = () => {
  ...
  const onSubmit: SubmitHandler<SignUpInputs> = async ({ emailAddress }) => {
    try {
      setIsLoading(true);
  ...
  const signInWithOAuth = async (strategy: any) => {
    ...
    try {
      ...
      if (error.response && error.response.status === 403) {
        ...
        // Notify the user in some way that the account already exists
